...
q_initial=randn(d,1);
[q,ne,fe] = root_of_dEp(q_initial,DATA_x,DATA_y,p);
DATA_i = linspace(min(DATA_x),max(DATA_x),4*N);
plot(DATA_i,polyval(q(end:-1:1),DATA_i),'r-','LineWidth',2);
text(0,4,sprintf('p = %0.2f error = %0.2f',p,fe));
disp(q);
disp(fe);
